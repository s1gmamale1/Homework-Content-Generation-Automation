...
with the key quantities/measurements (or, for evidence-based work, the key
observations) CHANGED, and ask the student to infer **which underlying concept
or method governs the new outcome**. The principle's name must NOT appear in the
setup; the `options` are candidate concepts/methods and `correct_option` is the
one the lesson actually supports. This tests transfer — recognising the unnamed
principle from a fresh instance, not re-running the forward steps. It counts as
one of the 2–4 `decisions`.
