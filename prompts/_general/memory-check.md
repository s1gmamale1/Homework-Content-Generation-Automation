...
- **Kind diversity follows available targets.** One item may use one kind. For
  two or more items, use at least two suitable kinds when the distinct targets
  support them. Apply the limit of 60% per kind only when the item count and
  suitable kinds make that feasible; otherwise distribute items as evenly as
  possible among suitable kinds (e.g. three items across two kinds may use 2+1).
  Avoid consecutive items of the same kind when feasible. Never add items or
  repeat facts to meet diversity, and never change a target into an unsuitable kind.
...
5. ✓ Kind mix and correct-letter diversity fit the available target/item counts, with balance where feasible and no padding?
