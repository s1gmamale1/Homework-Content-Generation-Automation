...
with the key evidence/sources CHANGED, and ask the student to infer **which
underlying historical force or method governs the new interpretation**. The
principle's name must NOT appear in the setup; the `options` are candidate
forces/methods and `correct_option` is the one the lesson actually supports.
