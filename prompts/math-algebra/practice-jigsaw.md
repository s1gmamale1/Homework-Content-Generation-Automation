# Prompt: Practice Game — Math + Algebra — Jigsaw Matching (Case-Based Preview interaction mode: Jigsaw Matching)

You are building the **Jigsaw Matching** practice game for a Math/Algebra homework session. It is a Case-Based Preview (CBP) interaction mode: the student plays a puzzle assembler / source checker / analyst who decides **which two source-supported pieces fit together**, **which assembly relationship type connects them**, and **why the tempting wrong combination cannot fit**.
...
- Surface similarity (same symbol, same shape, similar wording) is NOT a valid connection.
- Relationship DIRECTION matters: A defines/applies-to/produces B is not the same as B → A. A reversed assembly is wrong.
- Algebra assembly (piece pair) types — use a MAXIMUM of 3 per round, drawn from: **formula ↔ variable** (a formula and the quantity a symbol represents), **rule ↔ example** (a rule/identity and the instance that uses it), **step ↔ result** (a transformation step and the value it yields).
...
- Key terms MUST align with the lesson's Flashcards. Preserve every formula, symbol, number, and operation exactly.
...
- `case_setup`: `{ narrative, student_role, task }` — 2-4 sentences. The role must create a real need to assemble the correct piece pair to make a decision. Present a piece set of 3-6 source items (formulas, variables, rules, examples, steps, results).
...
- `decision_process_explanation` (DPE): comes AFTER checkpoint 3 and BEFORE the final simulation.
  - `prompt`: ask the student to walk through (1) which source concept they spotted, (2) why this assembly relationship label is correct (and its direction), (3) what mistake would occur with the wrong label or a weak/surface pair.
...
Low-asset: piece cards, assembly slots, type-label chips, a result puzzle. For formulas, variables, number lines, and relationship diagrams, embed a source-carrying inline SVG and follow the runtime universal SVG rules — no size, no colour, no decoration. Never render a formula as a raster image. SVG must carry the actual source relationship, never decorate it.

## Language

All student-facing text in formal Uzbek ("Siz"). Preserve formulas, symbols, numbers, and operations exactly.
...
5. ✓ Relationship source-supported, direction correct, surface similarity rejected?
