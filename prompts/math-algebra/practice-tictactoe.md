...
You are generating one **Case-Based Preview** practice game for this algebra
lesson, skinned as a **Tic-Tac-Toe Decision Grid**: a 3×3 board of action cells
where the student must pick the cell that correctly applies the lesson's formula,
operation, or solving strategy.
...
- `student_role` — solver / checker / analyst / advisor.
- `case_type` — e.g. `practical_decision_case` (choose the correct formula /
  operation / solving strategy).
...
- `case_setup` — `{narrative, student_role, task}`. A 2–4 sentence role-based
  scenario presenting a problem where the student must choose the **correct
  formula, operation, or solving strategy**. The decision must NOT be answerable
  by guessing — the lesson method is the only key. Describe the 3×3 decision board
  here (low-asset CSS/SVG), each cell a candidate move/strategy; embed a
  source-carrying SVG (expression, graph, before/after step) when it helps.
- `checkpoints` — EXACTLY 3, in this order:
  1. `intent: "identify"`, `form: "mcq"` — **which source concept (formula /
     rule / method) controls this problem?** Options: correct concept ·
     related-but-wrong concept · surface clue only · irrelevant detail. Set
     `correct_index`, write `feedback`.
  2. `intent: "decide"`, `form: "mcq"` — **the decision board.** "Qaysi katak
     ...
     correct strategy (applies the formula/operation) · plausible-but-incomplete
     strategy (right idea, missed step) · fast-but-unsafe strategy (shortcut that
     breaks a rule) · surface/irrelevant strategy. Set `correct_index`.
     `feedback` may name state-meter shifts (e.g. Aniqlik +30, Xato xavfi −20).
     Distractors must be tempting to a real learner, not silly.
...
- `final_simulation` — `{correct_path, wrong_path, why_wrong_fails}`. Correct path:
  the method-aligned strategy improves the visible state meters (Aniqlik, Xato
...
- `completion_rules` — `{pass_condition, retry_condition}`. Pass: identifies the
  ...
  picks a cell answerable without the lesson method, skips the explanation, or
...
Low-asset CSS/SVG only. Embed source-carrying inline SVG for expressions, graphs,
quantities, and before/after calculation steps, following the universal SVG rules
...
number, symbol, and unit exactly. Use the lesson's canonical algebra terms.
