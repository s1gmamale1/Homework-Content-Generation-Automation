...
- **Most content phases are markdown-only; two author `content_json` (worklog 0162).** The md-per-phase flip removed the old `STRUCTURED_PHASE_SCHEMAS` (phase→Pydantic) JSON-mode table, and every phase's markdown output is graded by the LLM judge (`phase_judge.py`, worklog 0037), which regenerates once on a MAJOR verdict. **Exception:** the phases in `app/schemas/content_json.SCHEMAS` — currently `practice-rlc` (`rlc_config@1`) and `practice-sentence` (`sentence_fill_config@1`) — author the platform's canonical `content_json` directly against `get_structured_prompt`, and their **markdown is RENDERED from that JSON** by `phase_render.py`, never authored. Those artifacts are graded on the canonical JSON against the structured contract (`_judge_inputs_for`); a schema-exhausted attempt falls back to markdown and records `authoring_mode="markdown_fallback"`. `run_phase`'s `schema=` JSON-validation mode (`model_validate_json` + one retry) serves that lane and structured callers like the judge; the remaining content phases still don't use it.
