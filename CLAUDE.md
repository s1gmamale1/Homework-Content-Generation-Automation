...
The single source of truth for accepted `(provider, model)` pairs. `/api/v1/agent/models` serves it with `api_supported`/`api_only` (provider-level) plus `api_only_models` (per-model, `{"gemini": sorted(GEMINI_API_ONLY_MODELS)}`). `API_PROVIDERS` is `{claude, gemini, clodex}` and `API_ONLY_PROVIDERS` is `{clodex}`; API calls require an explicit model, and Clodex+CLI is invalid. **`GEMINI_API_ONLY_MODELS`** (`gemini-3.6-flash`/`gemini-3.5-flash`/`gemini-3.5-flash-lite`) is a per-*model* cli reject within gemini (not a whole-provider one like `API_ONLY_PROVIDERS`) — these models aren't in the gemini CLI's own catalog, so `validate_transport` rejects `transport=cli` for them even though gemini otherwise supports cli. **`RETIRED_GEMINI_MODELS`** (`gemini-2.5-pro`/`-flash`/`-flash-lite`, retired 2026-08-03 — 404 on the plain API key) are removed from `MODEL_MANIFEST` (no longer offerable) but kept in `pricing.PRICE_MAP` + `model_tiers._MODEL_TIER` so historical `agent_usages` rows still resolve a real $/tier. A job pinned to a retired model before the cutover is guarded on every reactivation path — `app/services/job_reactivation.retired_models_in_job(job)` checks all four role pairs (content/extract/judge/solver) and is called from `jobs.py::retry_job` (409), and `batch.py`'s Resume (partial skip: `jobs_resumed`/`jobs_skipped_retired`) and relaunch-resume (409/block + FE consent dialog) loops — so retry/resume never silently re-fires a dead model.
...
- **Fleet-wide per-credential api concurrency limiter (BE-16 / `concurrency-knob-1` Phase 2, worklog 0142):** every `transport=api` call acquires a Postgres-backed slot (`credential_slots`, migration 0047) keyed by the credential it actually bills against (`credential_id.credential_for` — Vertex SA `gemini:{project}` / key fingerprint for gemini-AI-Studio/claude/clodex) before it runs, inside `agent._spawn_once`. Ceiling = env default (`CREDENTIAL_MAX_CONCURRENT_GEMINI`/`_CLAUDE`/`_CLODEX`, default 8, `0`=off) or, for a Vertex-SA gemini credential, the per-project override in the SA-keys panel (`sa_keys.max_concurrent_calls`, MIN across duplicate `project_id` rows). Saturation waits up to `credential_slot_wait_seconds` (120s) then surfaces as a `429`-shaped error into the existing rate-limit retry loop — never a hard failure. **Fail-open, loudly**: a limiter DB error logs `credential limiter: BYPASSED` (throttled ≤1/60s) and the call proceeds uncapped.
...
- **`extract` phase is pinned** to a role-level provider/model, resolved by `pipeline._resolve_extract`: the job's own `extract_provider`/`extract_model` if explicitly picked, else the `launch_defaults` singleton row's `extract_provider`/`extract_model` (current default `gemini`/`gemini-3.5-flash-lite`, tier4 — bumped off retired `gemini-2.5-flash` by the 2026-08-03 3.x-flash migration). There is no `settings.extract_provider`/`settings.extract_model` — that's a distinct setting, `settings.toc_validation_provider`/`_model` (default `gemini`/`gemini-3.5-flash`), used only by the post-TOC-extract vision validator. Extract is pinned regardless of which provider the user picked for the job's content phases — it's high-input/low-value (whole-PDF read → flat factual summary), so paying smart-tier rates buys nothing. All other phases honor `job.provider` / `job.model`. The **provider/model** pin is what's fixed per-role — the **auth mode** follows `job.transport`.
...
- A single phase sequence from `flows.flow_for(subject)` (`flows.py:43`): `_BASE_PHASES` + **all four interactive mini-games** (`_GAMES`) + `practice-sentence` — **6 content phases** (english prepends a 7th, `vocabulary`, 2026-08-25). **Homework-arc cut 2026-08-26 (owner):** memory-match/tictactoe/jigsaw/boss-arena/reflection left the flow (`flows._CUT_PHASES`, prompts dormant) — Free Practice mode / backend boss / retired reflection per the Akademiya standards. `SUBJECT_GAME` is now **metadata only** — the per-subject recommended-game hint, no longer gating the flow. **No `SUBJECT_FLOWS`, no easy/hard, no `classify`** — MVP single flow (`flows.py:1`).
...
- `books` — has legacy `gemini_file_uri` / `gemini_cache_*` columns that are unused but kept nullable for backwards-compat. The PDF lives on disk at `var/books/<book_id>/source.pdf`. `toc_ready_at` (migration 0048) stamps when a book last reached `toc_ready` — cleared on `/toc/retry`, set (not cleared) on `/toc/accept` — so the system-aware "Prepare a subject" dialog can tell an already-prepared book apart from a stale/never-extracted one.
- `book_notion_sources` — one row per Notion `(notion_page_id, notion_block_id)` -> `book_id` link (migration 0048, worklog 0144), `UNIQUE(notion_page_id, notion_block_id)`; a re-prepare that dedupes onto an existing book re-points the row via `notion_sources_repo.upsert_link` (`ON CONFLICT DO UPDATE`), never inserts a duplicate. Backfilled for pre-existing books by `scripts/backfill_notion_sources.py` (dry-run by default, matches by `content_sha256`+`subject`).
...
- **Gemini CLI rejects files > 20 MB**. TOC extraction is pinned to the `launch_defaults` extract role (`ld.extract_provider`/`ld.extract_model`, default gemini — NOT hardcoded; `toc_extractor.py` reads the same DB row as the lesson extract) and will fail for larger PDFs with a sandbox error. Pre-shrink, or change the `launch_defaults.extract_provider` row (claude consumes the PDF natively via positional `@path` argv).
...
- Don't add per-call provider/model overrides anywhere except where they already exist (extract pin via `job.extract_provider`/`job.extract_model` or the `launch_defaults` row); keeping job-level provider stable across the rest of the pipeline is what makes `agent_usages` and the UI badge mean something.
