...
- A single phase sequence from `flows.flow_for(subject)` (`flows.py:43`): `_BASE_PHASES` + **all four interactive mini-games** (`_GAMES`) + `boss-arena` + `reflection` — **11 content phases** (english prepends a 12th, `vocabulary` — the one subject-specific deviation, 2026-08-25), the full Gamified Practices set generated every job, none skipped (worklog 0067). `SUBJECT_GAME` is now **metadata only** — the per-subject recommended-game hint, no longer gating the flow. **No `SUBJECT_FLOWS`, no easy/hard, no `classify`** — MVP single flow (`flows.py:1`).
