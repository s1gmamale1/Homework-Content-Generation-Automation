...
_FROZEN_LANG_ENGLISH = (
    ...
    "CEFR ladder (one level per grade from G6): G5→A1, G6→A1, G7→A2, "
    "G8→B1, G9→B2, G10→C1, G11→C2. If no grade is visible, infer the "
    "level from the source's own complexity (default to A2 only if truly "
    ...
    "never exceed it.\n"
    "Level caps: A1 (G5–6) — ~800 words; sentences 3–8 words; present "
    "simple, can, there is/are, imperatives, present continuous basics. "
    "A2 (G7) — vocabulary inside the A2 Key list (~1,500 headwords); "
    "sentences 6–12 words; present simple/continuous, past simple, going "
    "to, comparatives, have to/should. B1 (G8) — vocabulary inside the B1 "
    "Preliminary list (~2,900 headwords); sentences 10–18 words; adds "
    "present perfect, first/second conditional, relative clauses, "
    "passive, reported speech (intro). B2 (G9) — ~4,000 headwords; adds "
    "all conditionals including mixed, full passive and reported speech, "
    "linking devices, opinion/argument register. C1 (G10) — ~8,000 "
    "headwords; nuanced register shifts, idiomatic collocations, advanced "
    "discourse markers, hedging and emphasis. C2 (G11) — no structural "
    "ceiling: precise, natural, sophisticated English with full idiomatic "
    "and stylistic range.\n"
    "BELOW B1 (grades 5–7): the thing being LEARNED is in English; "
    ...
    "language and no mother-tongue text anywhere, with exactly TWO "
    "exceptions: (1) the fixed translation lines the Topic Vocabulary "
    "phase itself defines, and (2) the machine-parsed labels each phase "
    "format defines, which stay EXACTLY as that format specifies at every "
    "level — `**To'g'ri javob:**` and `Noto'g'ri (<letter>):` keep that "
    "exact Uzbek form even in an all-English lesson; correctness tags "
    "follow each phase's own format section. This B1+ all-English rule "
    "OVERRIDES every other language directive, including the "
