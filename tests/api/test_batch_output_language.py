...
def _make_fake_ld(output_language="uz"):
    ...
    defaults to "cli" (GenerateRequest/BatchRequest schema default) — an "inherit" role
    transport would follow that cli content transport and fail validate_transport's
    cli-rejection. This matches migration 0049's actual target row (these roles stamped
    "api" unconditionally, not "inherit").
    """
    return SimpleNamespace(
