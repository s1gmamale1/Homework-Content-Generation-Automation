...
| `APP_GIT_REVISION` | The commit a build was made from, and a **precondition for creating a regeneration campaign** — the immutable `app_git_revision` audit column resolves explicit request field → this variable → the process's own git checkout → a structured `409 app_git_revision_unavailable` refusal (never NULL; blank counts as absent). CI bakes it into the image as a Docker build arg which the runtime re-exports, so a published image carries its own commit; a **hand-built image has neither that nor a `.git`** and cannot create campaigns until a build arg, a runtime env var, a real checkout or an explicit request field supplies one. No flag enables it. On a bare-metal head leave it unset — the env **outranks** the checkout, so a stale copy in a shared `.env` mis-stamps every campaign. |
