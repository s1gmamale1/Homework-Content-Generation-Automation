...
- `extract-1`: **TOC extraction still reads the PDF through the gemini CLI** (`agent.py` `_extract_toc_source_text` + the attached-PDF native read) — the SAME gitignore + broken-font (R10) risk the per-lesson extract phase just escaped in worklog [0035]. Reuse that effort's building blocks: read the TOC region's text **locally** (pypdf) and/or let the model locate by title, so TOC doesn't silently fail when gemini-cli can't read `var/`. Ties to R10's TOC manifestation.
