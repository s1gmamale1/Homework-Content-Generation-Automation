...
- **Issue:** the Phase-1 Notion push writes readable blocks, but a CBP `visual_svg` diagram (and markdown tables, e.g. teacher-note grids) shows up on the Homework page as raw escaped `<svg…>` / pipe-table text instead of an image/table. Ugly but non-blocking — **observed live 2026-06-02** on Homework page `37399838-1c76-81e8-a547-db74b8c1dbbc` (job `442cc7c4`, Geometriya 1.1). The lossless `homework.md` + `content.json` attachments still carry the real diagram, so no data is lost.
