...
- `claim_next_job` (`jobs.py:230-322`) ends in `.where(content_ok).where(judge_ok).where(extract_ok).order_by(...).with_for_update(skip_locked=True)` — the AND-compose point.
