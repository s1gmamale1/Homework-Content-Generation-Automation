> **DEFERRED 2026-07-16 (operator decision).** The operator's key turned out to be a **Clodex**
> key, not OpenAI — the lane pivoted mid-execution and shipped as the `clodex` provider
> (**PR #95**, squash `0172578`; plan file continued as `2026-07-15-clodex-api-provider.md`).
> Clodex absorbed this plan's shared machinery: the api-only validation class
> (`API_ONLY_PROVIDERS` + cli-rejection + `validate_role_provider` extract guard), the
> api_transport SDK branch (Task 1's `_openai` → `_clodex`), the provider stub / `API_PROVIDERS`
> dispatch / cli key scrub (Task 3, commits `482c7b2`/`c004b02`, in #95's squash).
> **Resuming OpenAI-official later = follow the clodex template**: new `openai` provider entry +
> `OPENAI_API_KEY`/`OPENAI_BASE_URL` client, live `models.list` probe for manifest ids, official
> price-page entries, worker capability flag + FE map additions. The task breakdown below is the
> reviewed original (fresh-Fable APPROVE-WITH-FIXES, all 7 fixes folded) — re-anchor every
> file:line against the post-#95 code before executing.
