...
4. Close CQ-C in `docs/memory/REMEDIATION_CLUSTERS.md` (Cluster 10) + R21 deliverable #2 (R21.2) in `docs/memory/ROADMAP.md` — **hand-merge the R21 list** (item 2 = CQ-C's line to close; item 6 = CQ-D's — don't duplicate/clobber). **Add ROADMAP follow-ups:** (a) the deferred FE — a `/settings` editor for the solver role + a `solver_status` badge on the Monitor/book views (backend works config-free via the seeded default; FE is enhancement, kept out of this backend-only PR to avoid the `web/` lane); (b) the **boss-arena wrong-feedback residue** (`263d99c5` Q3) uncovered by CQ-A+CQ-C — a future judge/boundary enhancement, since boss-arena emits no diffable key.
