...
**Goal:** One subject registry (`app/services/subjects.py`) that makes all 30 curriculum subjects first-class, with flows/prompts/notion_fetch/FE deriving from it. No regression for the 7 existing subjects.
...
- [ ] Run: `uv run python -m pytest tests/services/test_general_flow.py tests/services/test_learning_flow.py -q` → PASS (now iterate all 30).
...
- [ ] `Subject` union + `SUBJECTS` array = all 30 codes (existing 7 first, then new), with a comment: source of truth is `app/services/subjects.py`.
- [ ] `SUBJECT_LABELS: Record<Subject,string>` short English labels for all 30 (e.g. matematika:"Mathematics", ona-tili:"Uzbek", adabiyot:"Literature", russian:"Russian", geografiya:"Geography", informatika:"Informatics", huquq:"Law", iqtisodiyot:"Economics", astronomiya:"Astronomy", `chizmachilik`:"Technical drawing", etc.).
