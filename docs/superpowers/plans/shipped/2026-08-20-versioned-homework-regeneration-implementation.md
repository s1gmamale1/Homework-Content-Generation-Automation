...
Preflight reuses `_resolve_subject_page_id` and the existing lesson-title disambiguation. **Every target language's own `{lang}:{subject}|{grade}` mapping is required before any spend, even when the shared `toc_entries.notion_lesson_page_id` is populated** — that column is language-blind, and the publisher resolves each delivery beneath the target's own language subject page, so a lineage cannot be passed on the strength of a pointer another language filled in. It performs read-only validation and returns all missing mappings together. It makes no Notion writes and no model calls.
...
3. While the async DB session is open, load siblings with `toc_repo.titles_for_subject_grade(session, subject=job.subject, grade=book.grade)` and compute the collision-safe `lesson_title = resolve_lesson_title(section, siblings)`. Also resolve the language-aware subject-page ID and copy all scalar inputs needed remotely; close the DB session before Notion I/O. In one `asyncio.to_thread` call, resolve the Lesson Topic parent **beneath this target's own language subject page, on every delivery**: synchronously `find_or_create` that tree's existing `Generated Homeworks` container, then reuse `toc_entries.notion_lesson_page_id` **only after validating that it is a child of that container** (one `get_child_pages` membership check), and otherwise `find_or_create` the computed Lesson Topic title. The stored pointer is a hint, never the parent: it is a single language-blind column owned by whichever lineage archived first, so honouring it whenever present files one language's `Homework V{n}` under the other language's Lesson Topic — a `VersionPageCollision` that parks un-retryably, or a silent write beside the wrong V1. Validated membership keeps the pointer's real value within a language (a lesson whose disambiguating title suffix has since changed stays on the page it already has) without letting it carry a target across languages. No `NotionClientWrapper`, `find_or_create`, child listing, block read, upload, append, or clear call may run on the event loop. A crash before stamping is safe because the same collision-aware title path adopts the parent on retry. In a new fenced DB transaction, persist only `toc_entries.notion_lesson_page_id`.
