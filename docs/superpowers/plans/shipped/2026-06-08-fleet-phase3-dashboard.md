...
- [ ] **Step 2: `launcher.tsx`** — two parts in a `CARD`:
  - **Prepare form:** Grade `<select>` from `api.listNotionGrades()` (returns `NotionGrade[]`, each `{ title, page_id }` — render/select on `grade.title`) → Subject `<select>` from `api.listNotionSubjects(grade.page_id)` (returns `NotionSubject[]` `{ notion_title, page_id, app_subject, has_textbook }`; disable options where `!has_textbook`) → **Prepare** button (`PRIMARY_BTN`). On click: `useMutation(() => api.fetchBookFromNotion(subject.page_id, grade.title))`; on success `toast.success` + invalidate `["books"]`. **Mirror the existing call site `routes/upload.tsx:106`** — it calls `api.fetchBookFromNotion(s.page_id, nGrade)` where `nGrade` is the picked grade's `title` (set via `pickGrade(g.page_id, g.title)`); pass the grade in that same form so the endpoint gets what it already expects. (Both notion methods exist at `api.ts:208/213`.)
