...
- **Date:** 2026-06-16 (rev. 3 — folds in review rounds 1 & 2)
...
4. **Output caps + truncation are never silent.** Gemini: **no cap** (parity with today's CLI — the gemini CLI is invoked with no token-limit flag). Claude: a cap is **mandatory** (the Anthropic Messages API *requires* `max_tokens`) → new `settings.api_max_output_tokens`, default **16384** (sized with headroom over the longest *uncapped* content phases — e.g. reading / preview-hard, which carry no per-phase cap; the ~6k CBP is not the worst case — model-cap permitting). **Hitting the cap is detected and fails LOUD** (claude `stop_reason=="max_tokens"`; gemini `finish_reason==MAX_TOKENS`) → `rc=1`, never a silently-truncated success (§1, error handling). So even if a phase exceeds the cap it errors rather than shipping partial content — which also protects the api-vs-cli benchmark from being skewed by silently-short outputs. No `thinking_config` override in v1. ⚠ **Open for your nod:** the **16384** value (adjust freely — the fail-loud net makes the exact number low-risk).
...
- **`run_phase` retries a blank success** (`max_attempts=2`; `agent.py:704-706` treats `rc=0` + empty body — "gemini INVALID_STREAM" — as a failure and retries once). So mapping a blocked/empty SDK response to `(rc=0, "")` reuses this path. A **truncated non-empty** response is NOT empty, so it needs the explicit fail-loud branch (decision 4 / §1), not this one.
- **`_failure_preview` truncates stderr/text to 400 chars** (`agent.py:416`) before it's surfaced — so an auth/truncation signal must appear within the first 400 chars of the message (relevant to test #9).
- **Per-provider usage semantics** (must be preserved for correct pricing — `pricing._PROMPT_INCLUDES_CACHED = {gemini}`):
  - **gemini** (`providers/gemini.py:128-144`): `output = candidates + thoughts`; `prompt` INCLUDES cached; total = `total_token_count`. SDK `usage_metadata` → `prompt_token_count`, `candidates_token_count`, `thoughts_token_count`, `cached_content_token_count`, `total_token_count`.
  - **claude** (`providers/claude.py:75-95`): `prompt = input_tokens` (DISJOINT); `cached = cache_read_input_tokens`; `output = output_tokens`; **`total = input + output + cache_read + cache_creation`** (sum of present terms — must match the CLI provider so cli/api rows agree). SDK `message.usage` → `input_tokens`, `output_tokens`, `cache_read_input_tokens`, `cache_creation_input_tokens`.
- **Pricing is per-provider and transport-agnostic** (`pricing.cost_usd`, recomputes from prompt/cached); api rows record `auth_mode="api"` — **no pricing change**.
...
async def generate(*, provider: str, model: str | None, prompt: str,
    ...
    (returncode, text, usage, stderr). rc=0 + text = success; rc=0 + "" = blank
    (lets run_phase's empty-body retry fire); rc=1 = hard failure incl. TRUNCATION
    (stderr carries the cause)."""
```

- **Model guard** (both): falsy `model` → raise `ValueError("<provider> api requires an explicit model")`. Defense-in-depth (no live path sends `None`, but a blank `settings.extract_model` would otherwise reach the SDK as an opaque error).
- **Attachments guard** (both): non-empty → raise `NotImplementedError("api transport is text-only in v1")`.
- **gemini branch** (`from google import genai`): build client mirroring `_auth_env` (key → `genai.Client(api_key=…)`; else Vertex SA → `genai.Client(vertexai=True, project=…, location=…)`; else raise). `resp = await client.aio.models.generate_content(model=model, contents=prompt)` (native async; no cap; no `to_thread`). **Extract text defensively from `resp.candidates[0].content.parts`** — do **not** use `resp.text` (it raises on a blocked/empty candidate). Then classify by `finish_reason`:
  - `MAX_TOKENS` (even though gemini is uncapped, the model has its own hard limit) **with non-empty text** → `(1, "", <usage>, "output truncated: finish_reason=MAX_TOKENS")` (loud).
  - no usable text (safety block / `MAX_TOKENS` with no part / no candidate) → `(0, "", <usage>, "<finish_reason>")` (empty-body retry).
  - otherwise → `(0, text, <usage>, "")`.
  Map `usage_metadata` (thoughts folded into output, prompt cached-inclusive, total = `total_token_count`); store it in `raw`.
- **claude branch** (`from anthropic import AsyncAnthropic`): `ANTHROPIC_API_KEY` or raise. `msg = await client.messages.create(model=model, max_tokens=settings.api_max_output_tokens, messages=[{"role":"user","content":prompt}])`. Text = join of `block.text` for text blocks. Classify by `msg.stop_reason`:
  - `"max_tokens"` → `(1, "", <usage>, f"output truncated at max_tokens={settings.api_max_output_tokens}")` (loud — this *always* means the cap was hit, regardless of partial text).
  - empty text (non-truncation stop) → `(0, "", <usage>, "<stop_reason>")` (retry).
  - otherwise → `(0, text, <usage>, "")`.
  Map `msg.usage` (disjoint cached; total = input+output+cache_read+cache_creation, summing present terms); store in `raw`.
...
Add `api_max_output_tokens: int = 16384` to `app/config.py` (env `API_MAX_OUTPUT_TOKENS`) — used by the **claude** SDK branch only. Pydantic setting (a tuning knob, not an api credential — credentials stay `os.environ`).
...
- **Truncated output** — claude `stop_reason=="max_tokens"`, or gemini `finish_reason==MAX_TOKENS` with a non-empty partial → `rc=1` + clear `stderr` ("output truncated…"), a **LOUD failure** (→ same-provider retry, then job failure if persistent). Never a silent partial success. This is a *distinct* branch from the empty-body case — the empty-body retry only fires on `rc=0 + ""` and would not catch a non-empty partial.
...
3. **usage mapping:** gemini (thoughts→output, prompt cached-inclusive, total=total_token_count); claude (disjoint cached, **total=input+output+cache_read+cache_creation**); `raw` populated; **missing/partial usage → all-None empty dict (no crash)**.
...
6. **response classification:** blocked/empty (no candidate / MAX_TOKENS no-part) → `(0, "")`; **truncated non-empty** (claude `stop_reason=max_tokens`; gemini `finish_reason=MAX_TOKENS` + partial text) → `(1, "", …, "…truncated…")` — LOUD, **not** a silent `(0, partial)` success.
