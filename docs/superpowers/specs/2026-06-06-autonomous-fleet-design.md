...
- **Notion → lessons resolver (⚠ first-class, fallible — NOT a cheap reuse).** Our existing `notion_fetch.py` only goes subject → **one textbook PDF** (`download_textbook`); endpoints are just `/grades` + `/grades/{id}/subjects`. **There is no lesson enumeration.** Lessons = a book's `toc_entries`, which exist only after `download → ingest_pdf → async TOC extraction → list toc_entries`. So the resolver is a pipeline: (1) resolve subject → download + ingest + **extract TOC** (may need the >20 MB / scanned-book / glyph fallbacks — the project's most fragile step), (2) surface `toc_entries` as the lesson list, (3) operator picks. **TOC-extraction failure is a first-class state** the launcher and dashboard handle, not an exception.
  > **⚠ Phase 2 narrows this (intentional deviation — see `2026-06-08-fleet-phase2-batch-automation-design.md`).** The Phase 2 batch launcher is **fan-out only**: it consumes an already-`toc_ready` book (steps 1–2 stay owned by the existing `POST /books/from-notion` → `ingest_pdf` → `toc_extractor` flow, which already surfaces extraction failure first-class on the book). The launcher does **not** drive download/ingest/extract — it rejects a non-`toc_ready` book with a clear message and fans out one job per lesson. "Operator picks" (step 3) is the optional `toc_entry_ids` subset (default all); the picking *UI* is Phase 3. Re-wrapping the fragile pipeline into the batch was rejected as duplicating a working lifecycle for marginal gain.
...
- **Phase 0.5 — Clock-skew hardening (scoped follow-up; surfaced by Phase 0/1 review, VERIFIED against code).** Worker timing mixes a **DB-set** `scheduled_at` (`server_default NOW()`) with a **host-computed** `datetime.now()` claim filter (`jobs.py:222,226`). On a multi-PC fleet each worker's host clock is compared against the head DB's timestamps → uneven/delayed claiming (large skew → starvation). **Real fix: use DB-side `func.now()` for every timing comparison** so all reasoning happens on the single head-DB clock. **Verified full scope (7 spots, not just the claim):** (1) claim filter + `claimed_at`/`started_at`/`last_attempt_at` `jobs.py:226,241`; (2) `touch_claim` `:260`; (3) `reclaim_stuck_jobs` cutoff `:277`; (4) `reclaim_stale_cancelling` cutoff `:422`; (5) `mark_failed_with_retry` backoff `scheduled_at` `:336`; (6) `queue_depth` filter `:353`; (7) `workers.upsert_heartbeat` vs `is_online` `workers.py:31,26` (worker-host vs head-host — the 0048 liveness fragility). `completed_at` writes (`:320/371/405/427`) are record-only → leave. **Caveats:** DB-side interval arithmetic needs `func.now() - text("interval '…'")`, NOT `func.now() - timedelta`; after `.values(claimed_at=func.now())` the returned ORM object's `claimed_at` is DB-computed (callers read `.id`, so fine — but refresh anything that reads it). **Gate:** touches 0031 resilience code under every phase + the suite is DB-free → MUST add a real-DB test (ideally skew-simulating) + run the full suite + a smoke before shipping. **Severity: robustness, NOT blocking** — the ±0.7s seen in dev is Docker-Desktop VM-clock drift; an NTP-synced fleet sees <100ms. The *test* symptom (T1 flakiness) is already fixed (`0377b05`, scheduled_at pinned to the past); Phase 0.5 is the production-side fix.
