...
- **`flows.py`** — `SUBJECTS`/`SUBJECT_GAME` (derived from `subjects.REGISTRY`), `flow_for(subject)` (the 8-phase sequence), `PHASE_DEPS` (DAG deps) + `filter_prior_outputs`/`resolve_phase_deps`, per-phase output caps, SVG-stripping of prior outputs.
