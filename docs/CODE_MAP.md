...
- **`agent.py`** — **CLI router & primary LLM surface.** `run_phase` / `run_phase_prompt` (markdown phases, with the per-phase JSON-`schema` mode + one retry), `extract_toc`, `summarize_lesson` + `read_whole_book_text` (whole-book-text extract), `extract_lesson_context` (legacy PDF-attach path, not called by the pipeline), `record_cached_lesson_extract`; `_resolve_model` / `_PROVIDER_DEFAULT_MODEL` (only `claude` + `opencode` carry a default); `_PLACEHOLDER_RULES` + `_VISUAL_PHASES` (visuals are described placeholders, never `<svg>`); concurrency semaphore + usage recording.
