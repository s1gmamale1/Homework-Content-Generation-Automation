...
| `notion_lesson_page_id` | String(128) NULL | migration 0059 (worklog 0176) — the shared "Lesson Topic" Notion page id, parent of BOTH the Homework and Teacher Deck sub-pages; lets either deliverable adopt the page the other created (order-independent archival). **Language-blind** — owned by whichever lineage archived first — so the versioned-regeneration publisher treats it as a *hint only*: it resolves the Lesson Topic beneath the target's own `{lang}:{subject}\|{grade}` subject page every time, and honours this pointer solely after proving the page is a child of that language's `Generated Homeworks` container. Fill-once; never repointed or cleared |
...
| `revision_of_job_id` / `regeneration_target_id` | FK → homework_jobs / FK → regeneration_targets, both NULL, **ondelete=RESTRICT** | migration 0063: the versioned-regeneration marker pair. `revision_of_job_id IS NOT NULL` is the single unambiguous predicate that identifies a **revision job**, deliberately chosen over widening `kind` (a revision stays `kind='homework'` so it reuses the whole pipeline/phase/cost/download stack). `ck_homework_jobs_revision_pair` forces the two to be NULL or NOT NULL together; `uq_homework_jobs_regeneration_target_id` gives a target at most one revision job; `ix_homework_jobs_revision_of_job_id` indexes the lineage. RESTRICT (not CASCADE) because a target records a publication version that is consumed forever — a cascade would silently free it for reuse. |
| `ck_homework_jobs_revision_no_batch` | CHECK | migration 0063: `revision_of_job_id IS NULL OR batch_id IS NULL`. A revision is **never** a Fleet batch member, so batch rollups, adoption, resume and dedup queries stay untouched by regeneration — enforced in the schema rather than relying on callers remembering. |
| `session_limit_strategy` | String(16) NULL | migration 0063: NULL on an ordinary job (batch-then-global resolution unchanged). A revision has `batch_id=NULL` and so no batch row to resolve from, so it stores its **own concrete** value, copied from the campaign's already-resolved contract. CHECK accepts `pause\|switch\|inherit`, but a revision is separately refused `'inherit'` — re-resolving it at job-creation time would read the mutable fleet-wide default at a second moment and split one campaign across two run policies. |
...
| `slot_index` | Integer NOT NULL, `UNIQUE(credential, slot_index)` (`uq_credential_slots_credential_slot_index`, migration 0060) | which of the credential's `[0, limit)` slots this row holds — **this index is what enforces the ceiling** |
...
Acquire (`credential_limiter.acquire`) claims a slot in ONE statement, run in AUTOCOMMIT: pick a `slot_index` in `[0, limit)` that no *fresh* row holds (`acquired_at > now() - STALE_TTL_SECONDS`, `= 2 × per_attempt_timeout_seconds`) at random, and insert it; `ON CONFLICT (credential, slot_index) DO UPDATE ... WHERE acquired_at <= now() - ttl` takes over a *stale* slot in place (rotating `id`, so the crashed holder's late `release` can't delete its successor's row). No row back means either the credential is saturated (poll again — exponential backoff 1s→5s + jitter, until `credential_slot_wait_seconds`, default 120s, is exhausted, then `None`) or a concurrent acquirer won that index first (retry immediately, bounded).

**Why the ceiling is exact without a lock.** `UNIQUE(credential, slot_index)` makes more than `limit` rows in that index range physically impossible, so exactness is a property of the schema rather than of a critical section held across client round trips. This replaced `pg_advisory_xact_lock(hashtext(credential))` after a 2026-08 production incident: every host shares one Gemini key and so hashed to the *same* lock, serializing all slot acquisition fleet-wide — 75 connections blocked, longest wait 822s, only 54 of a 900-slot ceiling in use, workers unable to even heartbeat. Note that the obvious lock-free rewrites are **not** safe: a plain `INSERT ... SELECT ... WHERE (count) < :limit` is racy at READ COMMITTED (a probe overshot `limit=1` to 9 rows with 40 concurrent acquirers, and the overshoot grows with fleet size), and moving the advisory lock *inside* that statement is worse still — the statement's snapshot is taken before the lock is granted, so all 40 acquirers inserted. Release is a plain `DELETE ... WHERE id = :id` in a `finally` (shielded against a second cancellation, mirroring `worker.py`'s documented release craft) — a no-op if the row is already gone (double-release or already swept). **The DB is only touched at acquire/release/poll — never held during the model call itself.** A crashed holder's row ages out of the count after `STALE_TTL_SECONDS` and is deleted by `credential_limiter.sweep()`, called from the worker's periodic loop as its own step (not inside `_sweep_stuck_jobs`' transaction — a limiter-table error must never abort job reclaims).
...
### 3.14 `regeneration_campaigns` — one row per versioned-regeneration campaign (migration 0063)

The **immutable** specification of one regeneration run. Nothing here is edited after creation.

| Column | Type | Notes |
|---|---|---|
| `id`, `created_at`, `updated_at` | mixins | |
| `status` | String(48) NOT NULL, server_default `'draft'` | `draft → canary_running → awaiting_canary_approval → approved → bulk_running → attention_required → completed \| completed_with_abandonments \| rejected \| cancelled`. Terminal set = `completed`, `completed_with_abandonments`, `rejected`, `cancelled`. Derived from the member targets by `regeneration_states.roll_up_campaign` — except `rejected`, which is an operator decision the service sets directly. |
| `selection_spec` / `requested_phases` / `excluded_phases` | JSONB NOT NULL | the frozen operator input: books/TOC entries/languages, the phases asked for, and the auto-included phases explicitly dropped back to "copied" |
| `refresh_extraction` | Bool NOT NULL | default **false**. True re-runs `extract`, which makes every content phase downstream and therefore auto-included — near-full regeneration. |
| `exclusion_acknowledged` | Bool NOT NULL | the operator's explicit "I understand this packet may be internally inconsistent" for the broken dependency edges |
| `launch_contract` | JSONB NOT NULL | the **resolved** contract: every role's provider/model/transport plus a concrete session-limit strategy, read from `launch_defaults` + the fleet default **exactly once, at creation**. A draft placeholder (`null` role provider, `'inherit'` strategy) is refused at persistence. Every later wave copies this and may not re-resolve — a campaign runs in two waves separated by a human gate, so a second resolution would let an operator's edit between canary and bulk give one immutable campaign two different meanings. |
| `canary_size` | Integer NOT NULL, server_default `1` | how many targets form the canary (min 1 — the canary *is* the gate) |
| `estimated_cost_low_usd` / `estimated_cost_high_usd` | Double NULL | what was shown to the operator, kept for the estimate-vs-actual comparison |
| `app_git_revision` | String(64) NULL | prompt provenance: which deployed tree ran. Written **once** at creation and never corrected, so the API refuses to create a campaign it cannot name — resolution is explicit request field → `APP_GIT_REVISION` env (a container has no `.git`) → `code_version.GIT_SHA` → structured `409 app_git_revision_unavailable`. Blank normalizes to absent; 64 chars is exactly a full SHA-256 object name, so no real revision truncates. The column is nullable, but the create path never writes NULL — the 409 is what happens instead |
| `canary_launched_at` / `approved_at` / `rejected_at` / `cancel_requested_at` / `completed_at` | DateTime(tz) NULL | lifecycle audit |
| `rejected_reason` / `cancel_requested_reason` | Text NULL | |

### 3.15 `regeneration_targets` — one lesson + output language within one campaign (migration 0063)

Generation state and publication state are **separate** on purpose: a generated revision is never regenerated merely because Notion delivery failed.

| Column | Type | Notes |
|---|---|---|
| `id`, `created_at`, `updated_at` | mixins | |
| `campaign_id` / `toc_entry_id` | FKs NOT NULL | `ix_regeneration_targets_campaign_id` |
| `output_language` | String(8) NOT NULL | a lineage is `(toc_entry_id, output_language)`; UZ/RU/EN of one lesson have independent sources, locks and V2/V3 sequences |
| `source_job_id` | FK → homework_jobs NULL, **ondelete=SET NULL** | `ix_regeneration_targets_source_job_id`. **The one deliberately non-RESTRICT regeneration FK.** The restrictive half of the source-deletion rule lives on `fk_homework_jobs_revision_of_job_id` (RESTRICT), which forces a child-first order: a source can only be deleted once its revision job already is, and doing so **blanks this column instead of blocking**. Nullable so campaign reporting — the consumed `publication_version` and `notion_page_id` — survives that purge. A row with `source_job_id IS NULL` has no snapshot behind it; `lineage_targets_missing_source` detects exactly that and its callers refuse to regenerate from it. |
| `is_canary` | Bool NOT NULL, server_default `false` | only canary targets get a revision job before approval, so no worker can start bulk work before the human gate |
| `phase_plan` | JSONB NOT NULL | the planner's **serialized object**, not a bare phase-name list — the regenerated/copied split, the auto-included and acknowledged-excluded sets, the broken edges and the extraction flag all survive to the orchestrator, publisher and report with one agreed meaning. Written by `RegenerationPhasePlan.to_json` and read back **only** through `from_json`. |
| `status` | String(32) NOT NULL, server_default `'planned'` | `planned`, `generating`, `awaiting_canary_approval`, `publication_pending`, `publishing`, `published`, `generation_failed`, `publication_failed`, `abandoned` |
| `publication_released_at` | DateTime(tz) NULL | when approval made this target eligible for delivery |
| `publication_version` / `notion_page_id` | Integer NULL / String(128) NULL | the reserved version and the authoritative page id once created |
| `publication_claim_token` / `publication_claimed_at` | UUID NULL / DateTime(tz) NULL | the publisher's bounded lease; an expired lease is recoverable by another pass after a crash |
| `publication_attempts` / `publication_next_attempt_at` / `publication_last_error` | Integer NOT NULL / DateTime(tz) NULL / Text NULL | bounded exponential backoff state. `publication_next_attempt_at` is what splits a `publication_failed` row into `backing_off` / `retry_due` / `action_required` for the operator. |
| `terminal_at` / `terminal_reason` | DateTime(tz) NULL / Text NULL | **only** `published` and `abandoned` set `terminal_at`. `generation_failed` and `publication_failed` are attention-required, retryable, and keep `terminal_at=NULL`. |
| `abandon_requested_at` / `abandon_requested_reason` | DateTime(tz) NULL / Text NULL | audited give-up; never deletes a Notion page |

**The three constraints that carry the design:**

- **`uq_regeneration_targets_campaign_toc_language (campaign_id, toc_entry_id, output_language)`** — one target per lesson per campaign.
- **`uq_regeneration_targets_active_lineage (toc_entry_id, output_language) WHERE terminal_at IS NULL`** — at most one *non-terminal* target per lineage **across all campaigns**. Because `generation_failed`/`publication_failed` are non-terminal, a failed target **blocks** a competing campaign for that lesson until the operator retries it to `published` or explicitly abandons it.
- **`uq_regeneration_targets_publication_version (toc_entry_id, output_language, publication_version) WHERE publication_version IS NOT NULL`** — the final fence on version allocation, underneath the advisory lock in `reserve_publication_version`.

Plus CHECKs that a published target has both a version and a page id, and that no target reaches a publication state before its campaign is approved.

**Version allocation.** `reserve_publication_version` reserves atomically **when delivery first begins** — not at campaign or canary creation. So: the first allocated number is **2** (logical V1 is the pre-existing `Homework` page and has no row here); a rejected canary, a never-started target and a generation failure consume **nothing**; once publication begins the number is never reused, not by a failed delivery, a cancellation or a later campaign; and every retry publishes at the same reserved version and page identity. An **abandoned** target keeps its reserved number — it is permanently consumed and the next regeneration of that lesson publishes at the following version.

**Cost-cap scope (both directions, because it surprises people).** A revision job carries `batch_id=NULL` by CHECK, and `cost.batch_api_cost_usd` joins usage to a batch — so **no individual batch cost cap ever applies to a regeneration campaign**. `cost.fleet_api_cost_usd` has no job-kind filter — so **regeneration usage does count toward `COST_CAP_FLEET_DAILY_USD`** and a large campaign can trip it and pause ordinary api batches fleet-wide.

**Deletion is blocked, deliberately.** A target records a version consumed forever, so migration 0063's foreign keys are `RESTRICT` — `fk_regeneration_targets_toc_entry_id`, `fk_regeneration_targets_campaign_id`, `fk_homework_jobs_revision_of_job_id`, `fk_homework_jobs_regeneration_target_id`, `fk_phase_outputs_copied_from_phase_output_id` — with **one intentional exception: `fk_regeneration_targets_source_job_id` is `SET NULL`** (see the `source_job_id` row above). Rather than surface a raw `ForeignKeyViolation`, `app/api/v1/books.py` refuses first with a structured `409` naming the blocker: `book_delete_blocked_by_regeneration`, `toc_entry_delete_blocked_by_regeneration`, `toc_retry_blocked_by_regeneration`. Those refusals come from `history_for_toc_entry`/`history_for_book` matching **any** target row for the lesson, terminal or not, so the `SET NULL` exception does not weaken them. A future explicit child-first purge may lift the block: `revision_of_job_id` RESTRICT forces the revision child to be deleted first, after which deleting the source succeeds and nulls `source_job_id`. Cancellation/abandonment does not erase audit history.

Operator guide: [`docs/runbooks/versioned-homework-regeneration.md`](./runbooks/versioned-homework-regeneration.md).
...
| 60 | 0060_job_reclaims | `0060_job_reclaims` | adds `homework_jobs.reclaims` Integer NOT NULL server_default `0` — scheduling-failure counter kept separate from `attempts` so a stale-sweep reclaim of a job that never started a phase no longer consumes the execution retry budget (retry-accounting-1) |
| 61 | 0061_credential_slot_index | `0061_credential_slot_index` | adds `credential_slots.slot_index` Integer NOT NULL (backfilled densely per credential via `row_number()`, so in-flight slots survive the deploy) + `UNIQUE(credential, slot_index)`. Replaces the fleet-wide `pg_advisory_xact_lock(hashtext(credential))` in `credential_limiter.acquire` with one lock per slot — see §3.12 |
| 62 | 0062_cap_pause_provenance | `0062_cap_pause_provenance` | adds `batches.paused_cap_usd`/`paused_by` + `budget_state.api_paused_cap_usd`/`api_paused_by` — the effective cap and the worker id (`hostname:pid@sha`) that decided a cap pause, so an uneven cap rollout can no longer oscillate a batch between a strict and a lenient host (`worker._may_lift_cap_pause`). NULL = manual/pre-migration pause, which keeps the historical permissive reconcile |
| 63 | 0063_regeneration_campaigns | `0063_regeneration_campaigns` | versioned homework regeneration: creates `regeneration_campaigns` (§3.14) and `regeneration_targets` (§3.15, incl. the partial unique indexes `uq_regeneration_targets_active_lineage` and `uq_regeneration_targets_publication_version`); adds `homework_jobs.revision_of_job_id` + `regeneration_target_id` (self/target FKs both **ondelete=RESTRICT**, paired CHECK `ck_homework_jobs_revision_pair`, `UNIQUE(regeneration_target_id)`, `ix_homework_jobs_revision_of_job_id`), the CHECK `ck_homework_jobs_revision_no_batch` (a revision can never be a Fleet batch member), and `homework_jobs.session_limit_strategy` String(16) NULL + CHECK `pause\|switch\|inherit` (NULL on an ordinary job; a revision stores its own concrete value and is separately refused `'inherit'`). Ships behind `REGENERATION_ENABLED=false` — **HEAD** |
...
| `regeneration_enabled` | `false` | master flag for versioned homework regeneration; off ⇒ every `/api/v1/regeneration*` route 404s (404 not 403, so a stale SPA can't detect them) |
| `regeneration_publisher_enabled` | `false` | second, independent flag for the head-side loop that writes `Homework V{n}` Notion siblings — separate so generation can be exercised with delivery dark. `POST …/approve` and `POST …/retry-publication` return `409 publisher_disabled` while it is off, and `409 notion_unavailable` when it is on but the head has no usable Notion destination (`NOTION_ENABLED` + a well-shaped `NOTION_API_KEY`) — which is also a precondition for the loop starting at all. Enable on ONE designated head/API process (the claim protocol is safe if two run it anyway). |
| `regeneration_publisher_interval_seconds` | 30 | idle sweep interval; a pass that did work loops straight back, so a backlog drains at Notion's pace. `run_once` delivers **at most one** publication per pass (serial pacing). |
| `regeneration_publisher_lease_seconds` | 300 | durable publication claim lease; must stay well above a realistic Notion write (page + children + PDF upload). A publisher killed mid-delivery holds its target until this expires, then another pass adopts the page by its marker. |
| `regeneration_publisher_max_attempts` | 5 | automatic delivery attempts before a target parks in `publication_failed` for an operator (`ge=1`; `0` would mean "never publish", which is the flag's job) |
| `regeneration_publisher_backoff_base_seconds` / `_max_seconds` | 60 / 3600 | exponential backoff between delivery attempts, and its ceiling |
| `regeneration_launch_wave_size` / `_interval_seconds` | 4 / 60 | launch stagger for a campaign's bulk wave — same mechanism as the Fleet `batch_launch_wave_*` pair but deliberately more conservative, because a regeneration wave re-runs whole snapshots on top of whatever generation the fleet is already doing. Either at `0` disables staggering. |
