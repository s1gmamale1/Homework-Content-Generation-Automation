...
| `source_job_id` | FK → homework_jobs NULL, RESTRICT while a revision exists | `ix_regeneration_targets_source_job_id`. Nullable so campaign reporting survives an explicitly ordered child-first purge; deleting a source out from under a live revision fails cleanly. |
...
**Deletion is blocked, deliberately.** Every regeneration FK is RESTRICT because a target records a version consumed forever. Rather than surface a raw `ForeignKeyViolation`, `app/api/v1/books.py` refuses first with a structured `409` naming the blocker: `book_delete_blocked_by_regeneration`, `toc_entry_delete_blocked_by_regeneration`, `toc_retry_blocked_by_regeneration`. A future explicit child-first purge may lift this; cancellation/abandonment does not erase audit history.
