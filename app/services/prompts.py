...
_NOTATION_EXACT = (
    ...
    "math).\n"
    "- Use only standard KaTeX-safe commands: \\frac{a}{b}, subscripts "
    "$a_{n+1}$, powers $x^{2}$, \\sqrt{...}, \\cdot, \\times, \\pm, \\neq, "
    "\\leq, \\geq, \\approx, \\to, \\dots, Greek (\\alpha, \\pi, ...), "
    "\\oplus (astronomy's Earth subscript $T_{\\oplus}$), function names "
    "(\\sin, \\cos, \\tan, \\log, \\ln). No "
    "custom macros, no \\begin{...} environments, no \\text{...} — units and "
    "words stay OUTSIDE the span ($v = 20$ m/s), and never nest \\frac inside "
    "\\frac — restructure into sequential steps instead.\n"
