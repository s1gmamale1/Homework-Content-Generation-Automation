...
_LANG_ENGLISH = (
    ...
    "G9→A2+, G10→B1, G11→B1+. If no grade is visible, infer the level from the "
    ...
    "Level caps: A1 — ~800 words; sentences 3–8 words; present simple, can, "
    "there is/are, imperatives. A1+ — adds present continuous basics; ≤10 "
    "words. A2-early (G7 = Prepare 2 units 1–10) — vocabulary inside the A2 "
    "Key list (~1,500 headwords); sentences 6–12 words; present simple/"
    "continuous, adverbs of frequency, past simple, will (intro), can/can't — "
    "NOT yet past continuous or comparatives (those are G8). A2 (G8) — adds "
    "past continuous, comparatives/superlatives, going to, have to/should/may. "
    "A2+ (G9) — adds present perfect (ever/never/just/already/yet/for/since), "
    "first conditional, gerund after verbs, must/might, simple passive; "
    "reading texts to ~250 words. B1 (G10) — vocabulary inside the B1 "
    "Preliminary list (~2,900 headwords); sentences 10–18 words; adds second "
    "conditional, reported speech (intro), relative clauses, fuller passive, "
    "modals of deduction. B1+ (G11) — adds past perfect, used to, fuller "
    "reported speech and relatives; ~100-word writing tasks.\n"
    "BELOW B1 (grades 5–9): the thing being LEARNED is in English; everything "
    ...
    "mother-tongue text anywhere, with exactly ONE exception: the fixed "
    "translation lines the Topic Vocabulary phase itself defines. This B1+ "
