...
function Payload({
  mode,
  payload,
}: { mode: CbpModeGame["interaction_mode"]; payload: InteractionPayload }) {
  switch (mode) {
    case "memory_match": {
        ...
        <ReviewCard title="Pairs">
          <ul className="flex flex-col gap-1 text-sm text-(--color-ink-soft)">
            {p.pairs.map((pr, i) => (
              ...
              <li key={i}>
                <RichText inline>{pr.left}</RichText> ↔ <RichText inline>{pr.right}</RichText>
              </li>
            ))}
          </ul>
        </ReviewCard>
          ...
          <ul className="flex flex-col gap-1 text-sm text-(--color-ink-soft)">
            {p.pieces.map((pc) => (
              <li key={pc.id}>
                ...
                <RichText inline>{pc.content}</RichText>
              </li>
            ))}
          </ul>
      ...
      return (
        <ReviewCard title="Sentence">
          ...
          <ul className="mt-2 flex flex-col gap-1">
            {p.chips.map((c, i) => (
              <li key={c.label} className="flex items-start gap-2 text-sm text-(--color-ink-soft)">
                <Badge variant={c.is_correct ? "success" : "neutral"} size="sm">
                ...
                <span>
                  ...
                  {c.reason && (
                    <span className="text-(--color-ink-muted)">
                      {" "}
                      — <RichText inline>{c.reason}</RichText>
                    </span>
                  )}
                </span>
              </li>
        ...
        <ReviewCard title="Grid (3×3)">
          <div className="grid grid-cols-3 gap-1">
            {p.cells.map((c) => (
              <div
                key={c.label}
                className={`rounded-(--radius-sm) border p-2 text-center text-sm ${c.is_correct ? "border-(--color-success) text-(--color-success)" : "border-(--color-border) text-(--color-ink-soft)"}`}
              >
                <RichText inline>{c.label}</RichText>
              </div>
            ))}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {game.source_concept_ids?.map((id) => (
      ...
      <ReviewCard title="Explain your reasoning">
        ...
        {game.expected_reasoning_keywords && game.expected_reasoning_keywords.length > 0 && (
          <AnswerKey>
            <Labeled label="Keywords">{game.expected_reasoning_keywords.join(" · ")}</Labeled>
          </AnswerKey>
        )}
      </ReviewCard>
    </div>
